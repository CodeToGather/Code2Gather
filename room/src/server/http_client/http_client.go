...
func (c *HttpClient) PostWithAuthHeader(path string, data []byte, authHeader string) ([]byte, error) {
	log.Println("Making POST request with auth header to " + path)

	dataBuffer := bytes.NewBuffer(data)
	req, err := http.NewRequest("POST", path, dataBuffer)
	if err != nil {
		return nil, err
	}
